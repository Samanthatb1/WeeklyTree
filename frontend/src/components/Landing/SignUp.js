...
function SignUp() {
  ...
  async function submit() {
    ...
    for (const link of data) {
      const regex = /https:\/\/linktr\.ee\/[A-Za-z]+/i;
      if (!regex.test(link)) {
    ...
    <div class="signup_container">
      <div class="signup_email">
        ...
        {linkTrees.map((linkTree) => {
          return (
            <div className="signup_linktree" key={linkTree.id}>
              {linkTree.html}
            </div>
          );
        })}

        <button class="signup_add" onClick={addLinkTree}>
          Add Link +
        </button>
        <br />
        <button class="signup_submit" onClick={submit}>
          Submit
        </button>
        {error && (
          <p>
            Sorry, there was an error. Possibly your email or a link you
            provided is invalid
          </p>
        )}
      </div>
